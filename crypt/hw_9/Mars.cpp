...
void MARS::keyExpansion(const uint32_t* key, int n) {
    uint32_t T[15];
    ...
    for (int i = 5; i <= 35; i += 2) {
        ...
        K_[i] = w ^ (rol32(B[jj], (int)(K_[i - 1] & 31)) & mask);
    }
}

MARS::MARS(const std::vector<uint8_t>& key) {
    size_t bits = key.size() * 8;
    if (bits < 128 || bits > 448 || (bits % 32) != 0)
        throw std::invalid_argument("MARS: key must be 128-448 bits (multiples of 32)");

    int nw = (int)(key.size() / 4);
    uint32_t kw[14] = {};

    for (int i = 0; i < nw; i++) 
        kw[i] = toLE(key.data() + 4 * i);
    keyExpansion(kw, nw);
}

void MARS::encryptBlock(const std::vector<uint8_t>& input, std::vector<uint8_t>& output) const {
    if (input.size() != 16)
        throw std::invalid_argument("MARS: block must be 16 bytes");

    output.resize(16);

    uint32_t A = toLE(input.data() + 0);
    uint32_t B = toLE(input.data() + 4);
    uint32_t C = toLE(input.data() + 8);
    uint32_t D = toLE(input.data() + 12);
    ...
    for (int i = 0; i < 8; i++) {
        B = (B ^ SBOX[A & 0xFF]) + SBOX[(ror32(A, 8) & 0xFF) + 256];
        C = C + SBOX[ror32(A, 16) & 0xFF];
        D = D ^ SBOX[(ror32(A, 24) & 0xFF) + 256];
    ...
    for (int i = 0; i < 16; i++) {
        uint32_t R = rol32(rol32(A, 13) * K_[2 * i + 5], 10);
        uint32_t M = A + K_[2 * i + 4];
        M = rol32(M, (R >> 5) & 31);
        uint32_t L = rol32((SBOX[M & 0x1FF] ^ (R >> 5) ^ R), R & 31);
    ...
    for (int i = 0; i < 8; i++) {
        ...
        D = (D - SBOX[(rol32(A, 16) & 0xFF) + 256]) ^ SBOX[rol32(A, 24) & 0xFF];

        uint32_t newD = rol32(A, 24);
        A = B;
        B = C;
        C = D;
        D = newD;
    ...
    fromLE(A, output.data() + 0);
    fromLE(B, output.data() + 4);
    fromLE(C, output.data() + 8);
    fromLE(D, output.data() + 12);
}

void MARS::decryptBlock(const std::vector<uint8_t>& input, std::vector<uint8_t>& output) const {
    if (input.size() != 16)
        throw std::invalid_argument("MARS: block must be 16 bytes");
    ...
    for (int i = 7; i >= 0; i--) {
        uint32_t Aadj = ror32(D, 24);
        uint32_t Bnew = A;
        uint32_t Cnew = B;
        uint32_t Dnew = C;

        uint32_t Borig = Bnew ^ SBOX[(Aadj & 0xFF) + 256];
        uint32_t Corig = Cnew + SBOX[rol32(Aadj, 8) & 0xFF];
        uint32_t Dorig = (Dnew ^ SBOX[rol32(Aadj, 24) & 0xFF]) + SBOX[(rol32(Aadj, 16) & 0xFF) + 256];
    ...
    for (int i = 15; i >= 0; i--) {
        uint32_t Kr = K_[2 * i + 5];
        uint32_t Km = K_[2 * i + 4];

        uint32_t oldA = ror32(D, 13);
        D = C;
        C = B;
        B = A;
        A = oldA;

        uint32_t R = rol32(rol32(A, 13) * Kr, 10);
        uint32_t M = A + Km;
        M = rol32(M, (R >> 5) & 31);
        uint32_t L = rol32((SBOX[M & 0x1FF] ^ (R >> 5) ^ R), R & 31);
    ...
    for (int i = 7; i >= 0; i--) {
        ...
        uint32_t oldA = rol32(rotA, 24);
        uint32_t oldB = A;
        uint32_t oldC = B;
        uint32_t oldD = C;

        B = (oldB - SBOX[((oldA >> 8) & 0xFF) + 256]) ^ SBOX[oldA & 0xFF];
        C = oldC - SBOX[(oldA >> 16) & 0xFF];
        D = oldD ^ SBOX[((oldA >> 24) & 0xFF) + 256];
